...
class WP_Bootstrap_Navwalker extends \Walker_Nav_Menu {
	...
	public function start_lvl( &$output, $depth = 0, $args = array() ) {
		...
		$output .= "\n$indent<ul role=\"menu\" class=\" eduvibe-dropdown-menu\" >\n";
	}
	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		* a 0 if the strings are equal.
		...
		if ( 0 === strcasecmp( $item->attr_title, 'divider' ) && 1 === $depth ) {
			...
			if ($args->has_children && $depth === 0) { $class_names .= ' dropdown'; } elseif ($args->has_children && $depth > 0) { $class_names .= ' eduvibe-dropdown-submenu'; }
	...
	public static function fallback( $args ) {
		if ( current_user_can( 'edit_theme_options' ) ) {
			...
			echo '>';
			echo '<li><a href="' . esc_url( admin_url( 'nav-menus.php' ) ) . '" title="' . __( 'Add a menu', 'eduvibe' ) . '">' . esc_attr( 'Add a menu', 'eduvibe' ) . '</a></li>';
			echo '</ul>';
			if ( $container ) {
				echo '</' . esc_attr( $container ) . '>'; }
		}
